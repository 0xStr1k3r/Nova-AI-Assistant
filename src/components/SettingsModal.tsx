        ...
        <div className="flex gap-1 px-5 pt-3 pb-0 shrink-0">
          {tabs.map(t => (
            ...
            >
              ...
              {t.id === "profile" && local.userVoiceProfiles?.length > 0 && (
        ...
        <div className="flex-1 overflow-y-auto p-5 min-h-0 space-y-4">
          ...
          {tab === "profile" && (
            <div className="space-y-4">
              ...
              {/* Voice response mode */}
              <div className="space-y-1.5">
                <label className="text-[11px] text-slate-400 font-semibold uppercase tracking-wider block">Voice Response Mode</label>
                <div className="grid grid-cols-2 gap-2">
                  {(["all", "user"] as const).map(mode => (
                    <button
                      key={mode}
                      type="button"
                      onClick={() => setLocal({ ...local, voiceResponseMode: mode })}
                      className={`flex items-center gap-2.5 p-3 rounded-2xl text-left transition-all duration-200 ${
                        local.voiceResponseMode === mode ? "text-white" : "text-slate-500 hover:text-slate-300"
                      }`}
                      style={local.voiceResponseMode === mode ? {
                    ...
                    >
                      {mode === "all"
                        ? <Wifi size={14} className={local.voiceResponseMode === mode ? "text-violet-400" : "text-slate-600"} />
                        : <Fingerprint size={14} className={local.voiceResponseMode === mode ? "text-violet-400" : "text-slate-600"} />
                      }
                      <div>
                        <p className="text-xs font-semibold">{mode === "all" ? "All Voices" : "Registered Only"}</p>
                        <p className="text-[10px] text-slate-500 mt-0.5">{mode === "all" ? "Anyone can trigger Nova" : "Voice-verified access only"}</p>
                      </div>
                      {local.voiceResponseMode === mode && (
                        <Check size={12} className="text-violet-400 ml-auto shrink-0" />
                      )}
                    </button>
                  ))}
                </div>
                ...
                >
                  ...
                  <div className="flex items-center justify-between px-4 py-3 border-b border-white/5">
                    <div className="flex items-center gap-2">
                      ...
                      <p className="text-xs font-semibold text-white">Registered Voice Profiles</p>
                    </div>
                    <span className="text-[10px] text-slate-500 font-mono">{local.userVoiceProfiles?.length || 0} enrolled</span>
                  </div>

                  <div className="p-4 space-y-3">
                    {/* Profile list */}
                    {local.userVoiceProfiles && local.userVoiceProfiles.length > 0 ? (
                      <div className="space-y-2">
                        {local.userVoiceProfiles.map((p, idx) => (
                          <div
                            key={idx}
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <p className="text-[10px] text-slate-500 font-mono mt-0.5">
                                  {p.spectrum.length} bands · {p.sampleRate}Hz
                                </p>
                              </div>
                            </div>
                            <button
                              type="button"
                              onClick={() => {
                                const updated = local.userVoiceProfiles.filter((_, i) => i !== idx);
                                setLocal({ ...local, userVoiceProfiles: updated });
                                setRecordingStatus(`Profile "${p.name}" removed.`);
                              }}
                      ...
                      <div className="flex items-start gap-2.5 p-3 rounded-xl"
                        ...
                        <p className="text-[11px] text-amber-300 leading-relaxed">
                          No voice profiles yet. Enroll your voice below so Nova can verify your identity.
                        </p>
                      </div>
                    )}
                    ...
                    <div className="pt-2 border-t border-white/5 space-y-3">
                      <p className="text-[11px] text-slate-400 font-semibold uppercase tracking-wider">Enroll New Voice</p>
                      ...
                      {enrollStep === 0 ? (
                        <div className="flex gap-2">
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              if (exists) {
                                setRecordingStatus(`⚠ Profile for "${nameToRegister}" already exists.`);
                                return;
                              }
                              setEnrollStep(1);
                              accumulatedFramesRef.current = [];
                            ...
                            style={{ background: "linear-gradient(135deg, rgba(139,92,246,0.7), rgba(99,102,241,0.7))", border: "1px solid rgba(139,92,246,0.4)" }}
                          >
                            <Plus size={12} /> Enroll
                          </button>
                        </div>
                        ...
                        <div className="space-y-3">
                            ...
                            {[1, 2, 3].map(s => (
                              <React.Fragment key={s}>
                                <div className={`flex items-center justify-center w-6 h-6 rounded-full text-[10px] font-bold transition-all ${
                                  s < enrollStep  ? "bg-emerald-500/25 text-emerald-400 border border-emerald-500/40"
                                  : s === enrollStep ? "bg-violet-500/30 text-violet-300 border border-violet-500/50 animate-pulse"
                                  : "bg-white/5 text-slate-600 border border-white/8"
                                }`}>
                                  {s < enrollStep ? <Check size={10} /> : s}
                                </div>
                                {s < 3 && (
                                  <div className={`flex-1 h-0.5 rounded-full transition-all ${s < enrollStep ? "bg-emerald-500/40" : "bg-white/8"}`} />
                                )}
                              </React.Fragment>
                            ))}
                          ...
                          {/* Phrase card */}
                          <div className="p-3 rounded-xl text-center space-y-1"
                            style={{ background: "rgba(139,92,246,0.08)", border: "1px solid rgba(139,92,246,0.18)" }}>
                            <p className="text-[9px] text-slate-500 uppercase tracking-widest font-bold">Phrase {enrollStep} — Read aloud:</p>
                            <p className="text-sm text-white font-medium italic leading-snug">
                              "{ENROLL_SENTENCES[enrollStep]}"
                            </p>
                          </div>
                          ...
                          {recordingVoice && (
                            <div className="space-y-1">
                              ...
                              <p className="text-[10px] text-violet-400 text-center font-mono animate-pulse">
                          ...
                          <div className="flex gap-2">
                            <button
                              type="button"
                              disabled={recordingVoice}
                              onClick={() => recordVoiceStep(enrollStep)}
                              className="flex-1 py-2.5 rounded-xl text-xs font-semibold text-white flex items-center justify-center gap-2 transition-all disabled:opacity-50"
                              style={{ background: "linear-gradient(135deg, rgba(139,92,246,0.7), rgba(99,102,241,0.65))", border: "1px solid rgba(139,92,246,0.4)" }}
                            >
                              <Mic size={13} className={recordingVoice ? "animate-pulse text-red-300" : ""} />
                              {recordingVoice ? "Recording…" : `Record Phrase ${enrollStep} (4s)`}
                            </button>
                            <button
                              type="button"
                              disabled={recordingVoice}
                      ...
                      {/* Status message */}
                      {recordingStatus && (
                        <div className={`flex items-start gap-2 p-2.5 rounded-xl text-[11px] font-mono ${
                          recordingVoice    ? "text-violet-400 border border-violet-500/20 bg-violet-500/5"
                          : isStatusError   ? "text-red-400    border border-red-500/20    bg-red-500/5"
                          : isStatusSuccess ? "text-emerald-400 border border-emerald-500/20 bg-emerald-500/5"
                          : "text-slate-400 border border-white/8 bg-white/3"
                        }`}>
                          {recordingStatus}
                        </div>
                      )}
